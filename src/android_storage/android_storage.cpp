...
namespace AndroidStorage {
    ...
    AndroidOpenMode ParseOpenmode(const std::string& openmode) {
        ...
        } else if (openmode == "r+" || openmode == "r+b") {
...
#define FR(FunctionName, ReturnValue, JMethodID, Caller, JMethodName, Signature) F(JMethodID, JMethodName, Signature)
#define FS(FunctionName, ReturnValue, Parameters, JMethodID, JMethodName, Signature) F(JMethodID, JMethodName, Signature)
...
#undef FR
    ...
    bool CreateFile(const std::string& path) {
        if (create_file == nullptr) return false;
    ...
    bool CreateDir(const std::string& path) {
        if (create_dir == nullptr) return false;
        auto env = GetEnvForThread();
        auto directory = GetParentPath(path);
    ...
    int OpenContentUri(const std::string& filepath, AndroidOpenMode openmode) {
        if (open_content_uri == nullptr) return -1;
        // Check whether filepath is startsWith content
        if (filepath.rfind("content://", 0) != 0) return -1;
        const char *mode = "";
        switch (openmode) {
    ...
    std::vector<std::string> GetFilesName(const std::string& filepath) {
        ...
        if (get_files_name == nullptr) return vector;
        auto env = GetEnvForThread();
        jstring j_filepath = env->NewStringUTF(filepath.c_str());
        auto j_object = (jobjectArray)env->CallStaticObjectMethod(native_library, get_files_name, j_filepath);
    ...
    bool CopyFile(const std::string& source, const std::string& destination) {
        if (copy_file == nullptr) return false;
        auto env = GetEnvForThread();
        ...
        jstring j_destination_path = env->NewStringUTF(destination.c_str());
        jstring j_destination_filename = env->NewStringUTF(destination_filename.c_str());
        return env->CallStaticBooleanMethod(native_library, copy_file, j_source_path, j_destination_path, j_destination_filename);
    }

    bool RenameFile(const std::string& source, const std::string& destination) {
        if (rename_file == nullptr) return false;
        auto env = GetEnvForThread();
        ...
        return env->CallStaticBooleanMethod(native_library, rename_file, j_source_path, j_destination_path);
    }

#define FR(FunctionName, ReturnValue, JMethodID, Caller, JMethodName, Signature) F(FunctionName, ReturnValue, JMethodID, Caller)
